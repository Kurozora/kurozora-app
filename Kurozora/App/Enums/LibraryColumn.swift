...
// MARK: - Column
enum LibraryColumn: String, Codable, CaseIterable, Hashable {
	// MARK: - Cases
	/// The item's title. Always visible and always the first column.
	///
	/// When ``LibraryColumnPreferences/showPoster`` is `true`, the title cell also renders
	...
	func isApplicable(to kind: LibraryKind) -> Bool {
		switch self {
...
struct LibraryColumnPreferences: Codable, Hashable {
	// MARK: - Properties
	/// The left-to-right order of every column the user may interact with.
	var order: [LibraryColumn]

	/// A map of user-adjusted column widths.
	///
	/// Columns missing from the map use ``LibraryColumn/defaultWidth``.
	var widths: [LibraryColumn: CGFloat]

	/// The columns the user has explicitly hidden.
	var hidden: Set<LibraryColumn>

	/// A boolean that indicates whether the title cell renders a poster with inline
	/// rating, favorite, and reminder chrome.
	///
	/// When `true`, the ``LibraryColumn/rating``, ``LibraryColumn/favorite``, and ``LibraryColumn/reminder``
	...
	init(from decoder: Decoder) throws {
		...
		self.order = rawOrder.compactMap { LibraryColumn(rawValue: $0) }
		...
		self.widths = Dictionary(uniqueKeysWithValues: rawWidths.compactMap { key, value in
			LibraryColumn(rawValue: key).map { ($0, value) }
		})

		let rawHidden = (try? container.decode([String].self, forKey: .hidden)) ?? []
		self.hidden = Set(rawHidden.compactMap { LibraryColumn(rawValue: $0) })

		self.showPoster = (try? container.decode(Bool.self, forKey: .showPoster)) ?? true

		let missing = LibraryColumn.allCases.filter { !self.order.contains($0) }
	...
	init(order: [LibraryColumn], widths: [LibraryColumn: CGFloat], hidden: Set<LibraryColumn>, showPoster: Bool) {
		self.order = order
		self.widths = widths
		self.hidden = hidden
	...
	func visibleColumns(for kind: LibraryKind) -> [LibraryColumn] {
	...
	///
	/// - Returns: The user-adjusted width, or ``LibraryColumn/defaultWidth`` when none has been set.
	func width(for column: LibraryColumn) -> CGFloat {

//
//  KFMReShareTextEditorView.swift
//  Kurozora
//
...
final class KFMReShareTextEditorView: KView, KFeedMessageTextEditorViewProviding {
	...
	let dateLabel = KSecondaryLabel()
	let opCommentPreviewContainer = UIView()
...
// MARK: - Configuration
private extension KFMReShareTextEditorView {
	func configure() {
		self.configureView()
		...
		self.configureOPCommentPreviewContainer()
	...
	func configureOPCommentPreviewContainer() {
		self.opCommentPreviewContainer.translatesAutoresizingMaskIntoConstraints = false
		self.opCommentPreviewContainer.layoutMargins = UIEdgeInsets(top: 8, left: 8, bottom: 8, right: 8)
		self.opCommentPreviewContainer.theme_backgroundColor = KThemePicker.tableViewCellBackgroundColor.rawValue
		self.opCommentPreviewContainer.layerCornerRadius = 10
	}

	func configureOPProfileImageView() {
		self.opProfileImageView.translatesAutoresizingMaskIntoConstraints = false
		self.opProfileImageView.contentMode = .scaleAspectFit
	}
	...
	func configureViewHierarchy() {
		...
		self.commentPreviewContainer.addSubview(self.opCommentPreviewContainer)

		self.opCommentPreviewContainer.addSubview(self.opProfileImageView)
		self.opCommentPreviewContainer.addSubview(self.opUsernameLabel)
		self.opCommentPreviewContainer.addSubview(self.dateLabel)
		self.opCommentPreviewContainer.addSubview(self.opMessageTextView)
	...
	func configureConstraints() {
		NSLayoutConstraint.activate([
			...
			self.opCommentPreviewContainer.topAnchor.constraint(equalTo: self.commentTextView.bottomAnchor, constant: 8),
			self.opCommentPreviewContainer.leadingAnchor.constraint(equalTo: self.commentTextView.leadingAnchor),
			self.opCommentPreviewContainer.trailingAnchor.constraint(equalTo: self.commentTextView.trailingAnchor),
			self.opCommentPreviewContainer.bottomAnchor.constraint(equalTo: self.commentPreviewContainer.bottomAnchor, constant: -8),

			self.commentPreviewContainer.bottomAnchor.constraint(greaterThanOrEqualTo: self.profileImageView.bottomAnchor, constant: 20),

			self.opProfileImageView.topAnchor.constraint(equalTo: self.opCommentPreviewContainer.layoutMarginsGuide.topAnchor),
			self.opProfileImageView.leadingAnchor.constraint(equalTo: self.opCommentPreviewContainer.layoutMarginsGuide.leadingAnchor),
			self.opProfileImageView.widthAnchor.constraint(equalToConstant: 32),
			self.opProfileImageView.heightAnchor.constraint(equalToConstant: 32),

			self.opUsernameLabel.topAnchor.constraint(equalTo: self.opCommentPreviewContainer.layoutMarginsGuide.topAnchor),
			self.opUsernameLabel.leadingAnchor.constraint(equalTo: self.opProfileImageView.trailingAnchor, constant: 8),

			self.dateLabel.topAnchor.constraint(equalTo: self.opUsernameLabel.topAnchor),
			self.dateLabel.centerYAnchor.constraint(equalTo: self.opUsernameLabel.centerYAnchor),
			self.dateLabel.leadingAnchor.constraint(greaterThanOrEqualTo: self.opUsernameLabel.trailingAnchor, constant: 10),
			self.dateLabel.trailingAnchor.constraint(equalTo: self.opCommentPreviewContainer.layoutMarginsGuide.trailingAnchor),

			self.opMessageTextView.topAnchor.constraint(equalTo: self.opUsernameLabel.bottomAnchor, constant: 8),
			self.opMessageTextView.leadingAnchor.constraint(equalTo: self.opUsernameLabel.leadingAnchor),
			self.opMessageTextView.trailingAnchor.constraint(equalTo: self.dateLabel.trailingAnchor),
			self.opMessageTextView.bottomAnchor.constraint(equalTo: self.opCommentPreviewContainer.bottomAnchor, constant: -8),

			self.opCommentPreviewContainer.bottomAnchor.constraint(greaterThanOrEqualTo: self.opProfileImageView.bottomAnchor, constant: 20),

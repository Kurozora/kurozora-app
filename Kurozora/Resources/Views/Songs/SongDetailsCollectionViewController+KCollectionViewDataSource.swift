...
extension SongDetailsCollectionViewController {
	override func registerCells(for collectionView: UICollectionView) -> [UICollectionViewCell.Type] {
		return [
			SongHeaderCollectionViewCell.self,
	...
	override func configureDataSource() {
		...
		self.dataSource = UICollectionViewDiffableDataSource<SectionLayoutKind, ItemKind>(collectionView: collectionView) { [weak self] (collectionView: UICollectionView, indexPath: IndexPath, itemKind: ItemKind) -> UICollectionViewCell? in
			...
			case .rating(let songDetailRating):
				...
				if let stats = self.song.attributes.stats {
			...
			case .rateAndReview(let songDetailRateAndReview):
			...
			case .sosumi:
				...
				sosumiCollectionViewCell?.copyrightText = self.song.attributes.copyright
		...
		SectionLayoutKind.allCases.forEach { [weak self] songDetailSection in
			...
			case .lyrics:
				if self.plainLyrics != nil {
					self.snapshot.appendSections([songDetailSection])
					self.snapshot.appendItems([.lyrics], toSection: songDetailSection)
				}
			case .rating:
				self.snapshot.appendSections([songDetailSection])
				let ratingItems: [ItemKind] = SongDetail.Rating.allCases.map { songDetailRating in
					.rating(songDetailRating)
				}
				self.snapshot.appendItems(ratingItems, toSection: songDetailSection)
			case .rateAndReview:
				self.snapshot.appendSections([songDetailSection])
				let rateAndReviewItems: [ItemKind] = SongDetail.RateAndReview.allCases.map { songDetailRateAndReview in
					.rateAndReview(songDetailRateAndReview)
				}
				self.snapshot.appendItems(rateAndReviewItems, toSection: songDetailSection)
			case .reviews:
			...
			case .sosumi:
				if let copyrightIsEmpty = self.song.attributes.copyright?.isEmpty, !copyrightIsEmpty {
					self.snapshot.appendSections([songDetailSection])
					self.snapshot.appendItems([.sosumi], toSection: songDetailSection)
				}
			}
		}

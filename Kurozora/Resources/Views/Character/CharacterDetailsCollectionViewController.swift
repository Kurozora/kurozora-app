...
class CharacterDetailsCollectionViewController: DetailsCollectionViewController, SectionFetchable, TypedSegueHandling {
	// MARK: - Enums
	...
	/// The authenticated user's library state for the character.
	...
	override func fetchDetails() async {
		...
		async let reviewIdentityResponse = KService.reviews(for: characterIdentity).cursor(nil).limit(10).response()
		async let personIdentityResponse = KService.people(for: characterIdentity).limit(10).response()
		async let showIdentityResponse = KService.shows(for: characterIdentity).limit(10).response()
		async let literatureIdentityResponse = KService.literatures(for: characterIdentity).limit(10).response()
		async let gameIdentityResponse = KService.games(for: characterIdentity).limit(10).response()
		...
		guard self.character != nil else { return }
		...
		do {
			self.personIdentities = try await personIdentityResponse.data
			self.updateDataSource()
		} catch {
			print("-----", error.localizedDescription)
		}
	...
	/// Fetches the auth user's favorite and review overlays for the current character.
	private func fetchUserOverlays() async {
		...
		do {
			let overlayResult = try await KService
				.favoritesOverlay(forUser: userIdentity, kind: .characters, itemIDs: [characterID])
		...
		do {
			let overlayResult = try await KService
				.reviewsOverlay(forUser: userIdentity, kind: .characters, itemIDs: [characterID])
			...
			if case .modified(let response, let etag) = overlayResult {
				...
				libraryAttributes.note = try? await KService
					.notesOverlay(forUser: userIdentity, kind: .characters, itemIDs: [characterID])
	...
	override func writeAReviewContext() -> ReviewEditorContext? {
		guard let character = self.character else { return nil }
		return ReviewEditorContext(kind: .character(character), rating: self.libraryAttributes?.rating, review: self.libraryAttributes?.review, note: self.libraryAttributes?.note, isSpoiler: self.libraryAttributes?.isSpoiler ?? false, recommendation: self.libraryAttributes?.recommendation)
	}

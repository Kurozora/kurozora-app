...
extension Game {
	...
	private var outboxSeed: LibraryOutboxSeed {
		LibraryOutboxSeed(
			...
			airingDate: self.attributes.publicationDate,
	...
	/// Adds the game to the user's library with the given status.
	///
	...
	func addToLibrary(status: LibraryStatus) async {
		...
		await LibraryOutbox.shared.enqueueSetStatus(status, trackableID: self.id.rawValue, userSlug: slug, kind: .games, seed: self.outboxSeed)
	...
	func removeFromLibrary() async {
		guard let slug = User.current?.attributes.slug else { return }
		await LibraryOutbox.shared.enqueueRemove(trackableID: self.id.rawValue, userSlug: slug, kind: .games)
	}
	...
	func toggleFavorite(on viewController: UIViewController? = nil) async {
		...
		await LibraryOutbox.shared.enqueueSetFavorite(desired, trackableID: self.id.rawValue, userSlug: slug, kind: .games)
	...
	func toggleReminder(on viewController: UIViewController? = nil) async {
		...
		await LibraryOutbox.shared.enqueueSetReminder(desired, trackableID: self.id.rawValue, userSlug: slug, kind: .games)
	...
	func rate(using rating: Double, description: String?) async throws(APIError) -> Double? {
		...
		await LibraryOutbox.shared.enqueueRate(score: rating, description: description, trackableID: self.id.rawValue, userSlug: slug, kind: .games)
		return rating
	}
	...
	func deleteRating() async throws(APIError) -> Bool {
		...
		await LibraryOutbox.shared.enqueueDeleteRating(trackableID: self.id.rawValue, userSlug: slug, kind: .games)
		return true
	}
	...
	func markAsHidden(_ hidden: Bool) async {
		...
		guard let slug = User.current?.attributes.slug else { return }
		await LibraryOutbox.shared.enqueueSetHidden(hidden, trackableID: self.id.rawValue, userSlug: slug, kind: .games)
	}

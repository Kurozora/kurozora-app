...
//	*/
//	public func getThemes(completion completionHandler: @escaping (_ result: Result<[AppTheme], KKAPIError>) -> Void) {
//		let themesIndex = KKEndpoint.Themes.index.endpointValue
//		let request: APIRequest<AppThemeResponse, KKAPIError> = tron.codable.request(themesIndex)
//
...
//		- Parameter result: A value that represents either a success or a failure, including an associated value in each case.
//	*/
//	public func getDetails(forThemeID themeID: Int, completion completionHandler: @escaping (_ result: Result<[AppTheme], KKAPIError>) -> Void) {
//		let themesDetails = KKEndpoint.Themes.details(themeID).endpointValue
//		let request: APIRequest<AppThemeResponse, KKAPIError> = tron.codable.request(themesDetails)
//
//		request.headers = headers
//		if !self.authenticationKey.isEmpty {
//			request.headers.add(.authorization(bearerToken: self.authenticationKey))
